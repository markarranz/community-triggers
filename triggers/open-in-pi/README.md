...
Crucially, the watcher only triggers a turn while Pi is idle, so **your own messages always take priority**. When you talk to Pi it answers you first, and the instant it finishes the next batch of call activity arrives and it keeps listening. The poll loop lives for the whole session: a question from you never pauses or ends the watch, and batches keep coming until the call ends. The transcript files remain the source of truth, so if the watcher ever stalls, Pi just reads `transcriptions.jsonl` and `events.jsonl` directly.
