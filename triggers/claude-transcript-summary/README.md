...
The trigger writes a `summarize-in-claude.command` wrapper next to the transcript files and opens it in your preferred terminal. The wrapper runs as `#!/bin/zsh -li`, so `claude` resolves from the same interactive shell environment you get in a new terminal. No install location is hard-coded.
