...
When `call-transcription-started` fires, this trigger opens your preferred terminal and runs `tuple connect --harness cursor`. Connect resolves the call state, gives Cursor a context prompt, and points it at the live transcript — so Cursor catches up on everything said so far, then watches the call as it happens and acts as a sharp third pair.
